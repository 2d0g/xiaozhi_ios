...
final class OpusDecoderTests: XCTestCase {
	func testInit() throws {
		try AVAudioFormatTests.validFormats.forEach {
			_ = try OpusCodec.Decoder(format: $0)
		}

		try AVAudioFormatTests.invalidFormats.forEach {
			XCTAssertThrowsError(try OpusCodec.Decoder(format: $0)) { error in
	...
	func testCorruptedOpusDataThrowsInvalidPacketError() throws {
		...
		let opusDecoder = try OpusCodec.Decoder(format: opusFormat)
		...
		XCTAssertThrowsError(try opusDecoder.decode(corruptedOpusData)) { error in
			guard let opusError = error as? OpusCodec.Error else {
				XCTFail("Unable to cast error to OpusCodec.Error")
				return
			}

			XCTAssertEqual(opusError, OpusCodec.Error.invalidPacket)
		}
	}
}
