...
public extension OpusCodec {
	class Decoder {
		...
		public func reset() throws {
			let error = OpusCodec.Error(opus_decoder_init(decoder, Int32(format.sampleRate), Int32(format.channelCount)))
			if error != .ok {
				throw error
			}
		}
...
public extension OpusCodec.Decoder {
	func decode(_ input: Data) throws -> AVAudioPCMBuffer {
		try input.withUnsafeBytes {
			...
			if sampleCount < 0 {
				throw OpusCodec.Error(sampleCount)
			}
	...
	func decode(_ input: UnsafeBufferPointer<UInt8>, to output: AVAudioPCMBuffer) throws {
		...
		if decodedCount < 0 {
			throw OpusCodec.Error(decodedCount)
		}
