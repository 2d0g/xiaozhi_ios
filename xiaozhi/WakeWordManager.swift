...
class WakeWordManager: ObservableObject {
    ...
    func initializeSherpa() {
        guard let encoder = Bundle.main.path(forResource: "encoder", ofType: "onnx"),
              let decoder = Bundle.main.path(forResource: "decoder", ofType: "onnx"),
              let joiner = Bundle.main.path(forResource: "joiner", ofType: "onnx"),
              let tokens = Bundle.main.path(forResource: "tokens", ofType: "txt"),
              let keywords = Bundle.main.path(forResource: "keywords", ofType: "txt") else {
            print("!!! [WakeUp] 缺失模型文件，请确保已拖入 Xcode Targets")
            
            // 详细打印哪个文件找不到了，方便排查
            let files = ["encoder.onnx", "decoder.onnx", "joiner.onnx", "tokens.txt", "keywords.txt"]
            for f in files {
                let parts = f.components(separatedBy: ".")
                if Bundle.main.path(forResource: parts[0], ofType: parts[1]) == nil {
                    print("❌ 缺失文件: \(f)")
                }
            }
            return
        ...
        // 2. 配置 Sherpa-ONNX 参数
        // 这里的配置对标 Python 端的 WAKE_WORD_OPTIONS
        ...
        var config = sherpaOnnxKeywordSpotterConfig(
            ...
            maxActivePaths: 12,      // 极限优化：从 6 提高到 12，允许引擎深度搜寻弱信号
            numTrailingBlanks: 1,
            keywordsScore: 30.0,     // 回调到 30.0，配合超低阈值使用更稳定
            keywordsThreshold: 0.00001 // 极限优化：再次降低 10 倍，几乎不设任何进入门槛
        )
    ...
    func processAudio(samples: [Float]) {
        ...
        // 优化策略：将增益降低到 10 倍以避免波形畸变（Clipping），让 AI 看清更清晰的特征
        ...
        if Date().timeIntervalSince(lastLogTime) >= 5.0 {
            ...
            print("DEBUG [WakeUp] 状态：灵敏度探测中，原始能量: \(String(format: "%.4f", rms)), 10倍增益后: \(String(format: "%.4f", boostedRms))")
            lastLogTime = Date()
        }
